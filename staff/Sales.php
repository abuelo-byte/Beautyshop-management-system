...
/********************************************
 * 2) Fetch Products from `beautyshop` Table
 ********************************************/
$sql = "SELECT id, name, category, price, stock, subcategory
        FROM beautyshop
        ORDER BY id ASC";
...
<body>
    ...
    <script>
            ...
            fetch('process_sale.php', {
                ...
                .then(response => {
                    if (response.trim() === 'Success') {
                        ...
                        cart = [];
                        updateCart();
                        showNotification(`Sale completed: $${totalAmount.toFixed(2)} for ${itemCount} items`);
                    } else {
        ...
        function enhancedCheckout() {
            ...
            showNotification(`Sale completed: $${totalAmount.toFixed(2)} paid with ${paymentMethodName}`);
        }

        // Payment simulation
        function handleCashPayment(totalAmount) {
            const amountTendered = window.prompt(`Total: $${totalAmount.toFixed(2)}\nEnter amount tendered:`, totalAmount.toFixed(2));
            if (!amountTendered) {
        ...
        function handleCardPayment(totalAmount) {
            alert(`Processing card payment for $${totalAmount.toFixed(2)}...`);
        ...
        function handleMobilePayment(totalAmount) {
            alert(`Mobile payment for $${totalAmount.toFixed(2)}\nShow QR code to customer.`);
            setTimeout(() => {
        ...
        function exportSalesData() {
            ...
            csvContent += "Sale ID,Date,Time,Items,Subtotal,Tax,Total,Payment Method\n";
